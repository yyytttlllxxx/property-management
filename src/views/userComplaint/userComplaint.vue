  ...
  <el-main>
    ...
    <el-table :height="tableHeight" :data="tableList" border stripe>
      ...
      <el-table-column align="center" width="120" label="操作">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status == '0' && hasPerm('sys:myUserComplaint:do')" type="success"
            icon="el-icon-delete" size="small" @click="doBtn(scope.row)">处理</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    async doBtn(row) {
      if (row.status == "1") {
        this.$message.warning("该反馈已经处理，无需重复处理！");
        return;
      }
      let parm = {
