      ...
      <el-table :height="tableHeight" :data="tableList" border stripe>
        ...
        <el-table-column prop="payPowerStatus" label="缴费状态">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.payPowerStatus == '0'" type="danger" size="normal">未缴费</el-tag>
            <el-tag v-if="scope.row.payPowerStatus == '1'" type="success" size="normal">已缴费</el-tag>
          </template>
        </el-table-column>
        <template slot="empty">
          <div class="empty-state">
            <div class="empty-planet"></div>
            <div class="empty-title">暂无电费记录</div>
...
export default {
  ...
  methods: {
    ...
    // 刷新按钮/获取列表
    async getList() {
      const res = await getMyPowerFeeApi(this.parms);
      if (res && res.code === 200) {
